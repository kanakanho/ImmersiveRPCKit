...
struct TransformationMatrixPreparationImmersiveView: View {
    ...
    private let rightFingerEntity: ModelEntity = .generateSphere(
        name: "R", color: SimpleMaterial.Color.white)
    private let indexFingerTipGuideBall: ModelEntity = .generateSphere(
        name: "indexFingerTipGuideBall",
        color: UIColor(red: 0.0, green: 1.0, blue: 0.0, alpha: 0.4), radius: 0.02)
        ...
        RealityView { content in
            ...
            if let unapprovedCapabilities, unapprovedCapabilities.anchor.contains(.hand) {
                ...
                let indexFingerTipAnchor = AnchorEntity(
                    .hand(AnchoringComponent.Target.Chirality.right, location: .indexFingerTip),
